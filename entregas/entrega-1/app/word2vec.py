...
def naiveSoftmaxLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset):
    ...
    # Calcula as probabilidades usando a função softmax
    y_hat = softmax(np.dot(outsideVectors, centerWordVec))

    # Cálculo da perda (loss)
    loss = -np.log(y_hat[outsideWordIdx])

    # Criar o vetor one-hot para a palavra externa correta
    y = np.zeros_like(y_hat)
    y[outsideWordIdx] = 1

    # Gradientes em relação ao vetor da palavra central
    gradCenterVec = np.dot((y_hat - y), outsideVectors)

    # Gradientes em relação aos vetores das palavras externas
...
def negSamplingLossAndGradient(
    ...
    # Inicializar as variáveis de loss e gradientes
    loss = 0.0
    gradCenterVec = np.zeros_like(centerWordVec)
    gradOutsideVecs = np.zeros_like(outsideVectors)

    # Primeiro, computa o custo e gradiente para a palavra externa correta (positive sample)
    z = sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))
    loss -= np.log(z)
    gradCenterVec += (z - 1) * outsideVectors[outsideWordIdx]
    gradOutsideVecs[outsideWordIdx] += (z - 1) * centerWordVec

    # Agora, para as palavras negativas (negative samples)
    for k in negSampleWordIndices:
        z = sigmoid(np.dot(outsideVectors[k], centerWordVec))
        loss -= np.log(1 - z)
        gradCenterVec += z * outsideVectors[k]
        gradOutsideVecs[k] += z * centerWordVec
...
def skipgram(
    ...
    # Obtenha o índice da palavra central
    centerWordIdx = word2Ind[currentCenterWord]
    centerWordVec = centerWordVectors[centerWordIdx]

    # Para cada palavra de contexto, calcula a perda e os gradientes
    for outsideWord in outsideWords:
        outsideWordIdx = word2Ind[outsideWord]
        curr_loss, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(centerWordVec, outsideWordIdx,
